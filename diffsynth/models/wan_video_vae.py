...
class VideoVAE_(nn.Module):
    ...
    def encode(self, x, scale, use_cache=True):
        if not use_cache:
             out = self.encoder(x)
    ...
    def decode(self, z, scale, use_cache=True):
        if isinstance(scale[0], torch.Tensor):
...
class WanVideoVAE(nn.Module):
    ...
    def tiled_decode(self, hidden_states, device, tile_size, tile_stride):
        ...
        for h, h_, w, w_ in tqdm(tasks, desc="VAE decoding"):
            ...
            # import pdb; pdb.set_trace()
            hidden_states_batch = self.model.decode(hidden_states_batch, self.scale, use_cache=self.use_cache).to(data_device)

            if hidden_states_batch.shape[2] > out_T:
    ...
    def single_encode(self, video, device):
        ...
        x = self.model.encode(video, self.scale, use_cache=True)
        return x


    def single_decode(self, hidden_state, device):
        hidden_state = hidden_state.to(device)
        video = self.model.decode(hidden_state, self.scale, use_cache=self.use_cache)
